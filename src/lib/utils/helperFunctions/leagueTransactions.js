...
import { getLeagueTransactions as getLeagueTransactionsApi } from '$lib/utils/platformApi';
...
const combThroughTransactions = async (week, currentLeagueID) => {
        ...
        for(const singleLeagueID of leagueIDs) {
                while(week > 0) {
                        transactionPromises.push(getLeagueTransactionsApi(singleLeagueID, week));
                        week--;
                }
                week = 18;
        }

        const transactionsDataJson = await waitForAll(...transactionPromises).catch((err) => { console.error(err); });
