...
/**
 * A past season's final rosters in the archive row shape. Reuses the existing
 * roster adapter (which fetches each team's roster and already tolerates a single
 * team failing — bad teams come back with empty players rather than aborting) and
 * maps fields exactly the way the live capture does. A total failure (e.g. Yahoo
 * refusing the whole collection) returns [] so the rest of the season's archive
 * still gets written.
 */
export async function fetchSeasonRosters(yahooClient, leagueKey) {
        try {
                const rosters = await getYahooLeagueRosters(leagueKey, yahooClient);
                return buildRosterRows(rosters);
        } catch (err) {
                console.error('[historyBackfill] roster fetch failed for', leagueKey, err?.message);
                return [];
        }
}

/**
 * Full archive payload for one season's league: header, real standings, every
 * week's real matchup scores, and each team's final roster. Weeks are fetched
 * with light concurrency.
 */
export async function fetchSeasonArchiveData(yahooClient, leagueKey) {
        ...
        const rosters = await fetchSeasonRosters(yahooClient, leagueKey);

        return { meta, standings, sides, rosters };
}
