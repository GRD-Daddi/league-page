...
function convertLeagueDataToSleeperFormat(meta, settings, leagueKey) {
        try {
                ...
                return {
                        ...
                        season_type: 'regular',
                        status: getSleeperLeagueStatus(yahooLeague),
                        sport: 'nfl',
                        ...
                        metadata: {
                                ...
                                is_finished: isYahooSeasonFinished(yahooLeague) ? 1 : 0
                        }
                };
        } catch (error) {
                console.error('[Yahoo Adapter] Error in convertLeagueDataToSleeperFormat:', error.message);
                return {
                        league_id: leagueKey,
                        name: 'Unknown League',
                        ...
                        roster_positions: [],
                        total_rosters: 0,
                        metadata: { yahoo_league_key: leagueKey }
                };
        }
}
...
// Yahoo's draft_status never advances to "complete" on its own once a season
// ends; the finished-season signal lives on the league resource as is_finished
// (1 when the season is over). Treat that as the Sleeper "complete" status so
// end-of-season logic (records, awards, champion detection) works correctly.
function isYahooSeasonFinished(yahooLeague) {
        const flag = yahooLeague?.is_finished;
        return flag === 1 || flag === '1' || flag === true;
}

function getSleeperLeagueStatus(yahooLeague) {
        if (isYahooSeasonFinished(yahooLeague)) return 'complete';
        return getSleeperStatus(yahooLeague?.draft_status);
}

function getWaiverType(waiverRule) {
        const typeMap = { 'faab': 2, 'continual': 0, 'gametime': 1 };
...
function convertScoringSettings(statCategories) {
        ...
        const yahooToSleeperStatMap = {
                '5': 'pass_yd', '6': 'pass_td', '7': 'pass_int',
                '9': 'rush_yd', '10': 'rush_td', '11': 'rec',
                '12': 'rec_yd', '13': 'rec_td', '15': 'st_td',
                '16': 'st_ff', '18': 'fum_lost', '19': 'fg_made',
                '20': 'fg_miss', '21': 'xp_made', '22': 'xp_miss',
                '32': 'def_td', '45': 'def_st_td', '57': 'fum_rec_td',
                '58': 'bonus_pass_yd_300', '59': 'bonus_pass_yd_400',
                '60': 'bonus_rush_yd_100', '61': 'bonus_rush_yd_200',
                '62': 'bonus_rec_yd_100', '63': 'bonus_rec_yd_200',
        };
...
function convertRosterPositions(rosterPositions) {
        ...
        const yahooToSleeperPosMap = {
                ...
                'W/R': 'FLEX', 'W/R/T': 'FLEX', 'Q/W/R/T': 'SUPER_FLEX',
                'K': 'K', 'DEF': 'DEF', 'D': 'DEF', 'BN': 'BN', 'IR': 'IR'
        };
