	...
	<div class="sn-container">
		...
		{:else}
			<div class="rivalry-selectors">
				<select class="sn-select" value={teamA} on:change={onA} aria-label="Select first team">
					{#each teamNames as name}
						<option value={name}>{name}</option>
					{/each}
				</select>
				<div class="vs-pill">VS</div>
				<select class="sn-select" value={teamB} on:change={onB} aria-label="Select second team">
					{#each teamNames as name}
						<option value={name}>{name}</option>
					{/each}
				</select>
			...
			{:else}
				<div class="matchup">
					<div class="sn-card sn-card-gradient sn-card-pad team-card cyan">
						<div class="sn-avatar xl">{initials(teamA)}</div>
					...
					<div class="sn-card sn-card-gradient sn-card-pad team-card purple">
						<div class="sn-avatar xl">{initials(teamB)}</div>
							...
							<table class="sn-table">
								<thead>
									<tr>
										...
										<th class="right">{teamA}</th>
										<th class="right">{teamB}</th>
								...
								<tbody>
									{#each meetings as m}
										<tr>
											...
											<td class="right sn-num" class:lime={m.pointsA > m.pointsB}>{fmt(m.pointsA)}</td>
											<td class="right sn-num" class:lime={m.pointsB > m.pointsA}>{fmt(m.pointsB)}</td>
											<td class="center sn-team-name">
												{m.pointsA === m.pointsB ? 'Tie' : m.pointsA > m.pointsB ? teamA : teamB}
											</td>
										</tr>
									{/each}
								</tbody>
				...
				{:else}
					<div class="sn-empty" style="margin-top:36px;">
						...
						<p>{teamA} and {teamB} haven't faced off in any archived season yet.</p>
					</div>
				{/if}
			{/if}
		{/if}
