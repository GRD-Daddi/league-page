...
//
//  1. resolveSeasonLeagueKey / loadAllTransactions only write to the cache when
...
const { resolveSeasonLeagueKey, loadAllTransactions } = await import('./+page.server.js');
...
describe('resolveSeasonLeagueKey caching boundary', () => {
	it('returns null and never enumerates when there is no yahooClient', async () => {
		const out = await resolveSeasonLeagueKey(null, 2021, 'userA');
		expect(out).toBeNull();
	...
	it('resolves but does NOT cache when no session scope is present (logged-out path)', async () => {
		...
		const first = await resolveSeasonLeagueKey(client, 2021, null);
		const second = await resolveSeasonLeagueKey(client, 2021, null);
	...
	it('caches a genuine hit only when both client and scope are present', async () => {
		...
		const first = await resolveSeasonLeagueKey(client, 2022, 'userCache');
		const second = await resolveSeasonLeagueKey(client, 2022, 'userCache');
	...
	it('never caches a null (unresolvable) season key', async () => {
		...
		const first = await resolveSeasonLeagueKey(client, 2030, 'userNull');
		const second = await resolveSeasonLeagueKey(client, 2030, 'userNull');
	...
	it('isolates cache entries per session id', async () => {
		...
		await resolveSeasonLeagueKey(client, 2023, 'userA');
		expect(enumerateLeagueSeasons).toHaveBeenCalledTimes(1);

		// userB must NOT be served userA's cached entry — its own enumeration runs.
		await resolveSeasonLeagueKey(client, 2023, 'userB');
		expect(enumerateLeagueSeasons).toHaveBeenCalledTimes(2);
...
describe('loadAllTransactions caching boundary', () => {
	it('does NOT cache when no session scope is present (logged-out path)', async () => {
		const first = await loadAllTransactions(client, 'nfl.l.aaa', null);
		const second = await loadAllTransactions(client, 'nfl.l.aaa', null);
	...
	it('caches a non-empty result only when both client and scope are present', async () => {
		const first = await loadAllTransactions(client, 'nfl.l.bbb', 'userTx');
		const second = await loadAllTransactions(client, 'nfl.l.bbb', 'userTx');
	...
	it('never caches an empty result (possible auth error)', async () => {
		...
		const first = await loadAllTransactions(client, 'nfl.l.ccc', 'userEmpty');
		const second = await loadAllTransactions(client, 'nfl.l.ccc', 'userEmpty');
	...
	it('never caches when loadLeagueData returns null (auth error)', async () => {
		...
		const first = await loadAllTransactions(client, 'nfl.l.ddd', 'userAuthErr');
		const second = await loadAllTransactions(client, 'nfl.l.ddd', 'userAuthErr');
	...
	it('isolates cache entries per session id', async () => {
		...
		await loadAllTransactions(client, 'nfl.l.shared', 'userA');
		expect(loadLeagueData).toHaveBeenCalledTimes(1);

		// userB must NOT be served userA's cached transactions — its own fetch runs.
		await loadAllTransactions(client, 'nfl.l.shared', 'userB');
		expect(loadLeagueData).toHaveBeenCalledTimes(2);
