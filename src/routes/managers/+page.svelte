...
<div class="sn-page">
	...
	<div class="sn-container">
		{#if careers.length}
			<div class="sn-grid-3">
				{#each careers as c}
					<a class="sn-card sn-card-pad manager-card" href={`/manager?team=${encodeURIComponent(c.teamName)}`}>
						<div class="manager-top">
							<div class="sn-avatar xl">{initials(c.teamName)}</div>
							{#if c.titles > 0}
						...
						<div class="manager-name-block">
							...
							<div class="sn-team-meta">{c.seasons} season{c.seasons === 1 ? '' : 's'} · best finish #{c.bestFinish ?? '—'}</div>
						</div>
